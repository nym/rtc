...
describe('ingest-claude-code hooks (subprocess integration)', () => {
  ...
  test('on-stop aggregates the transcript and emits tokens.consumed + worker.despawned', async () => {
  ...
  test('on-stop tolerates a missing transcript_path (still despawns)', async () => {
    const sessionId = 'sess-stop-2';
    await seedWorker(sessionId);
    const result = await runHook('on-stop.ts', { session_id: sessionId }, handle.port);
    expect(result.exitCode).toBe(0);
    expect(handle.state().workers[sessionId]?.alive).toBe(false);
